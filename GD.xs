 ...
 * normally when the macro is expanded. Do NOT do this for names libgd
 * already #defines (e.g. GD_UHDR_SUCCESS) - that would hide their numeric
 * value behind an unresolvable self-reference. */
#ifdef HAVE_IMAGEQUANT
# define GD_QUANT_DEFAULT GD_QUANT_DEFAULT
# define GD_QUANT_JQUANT GD_QUANT_JQUANT
# define GD_QUANT_NEUQUANT GD_QUANT_NEUQUANT
# define GD_QUANT_LIQ GD_QUANT_LIQ
#endif
#ifdef HAVE_UHDR
# define GD_UHDR_FORMAT_JPEG GD_UHDR_FORMAT_JPEG
...
#define GDIMAGECREATEFROMJXL(x)   gdImageCreateFromJxl((FILE*)x)
#  endif
#else
#  ifdef USE_PERLIO
...
#define GDIMAGECREATEFROMJXL(x) gdImageCreateFromJxl(PerlIO_findFILE(x))
#  else
...
#define GDIMAGECREATEFROMJXL(x) gdImageCreateFromJxl(x)
#  endif
#endif
...
#ifdef HAVE_UHDR
/* Format a gdUhdrError into a Perl croak(). Never returns. */
static void
croak_uhdr(const char *what, gdUhdrErrorPtr err) {
  if (err != NULL && err->message[0] != '\0')
    croak("%s: %s (code %d, provider code %d)",what,err->message,err->code,err->provider_code);
  else if (err != NULL)
    croak("%s failed (code %d, provider code %d)",what,err->code,err->provider_code);
  else
    croak("%s failed",what);
}
#endif
...
#ifdef HAVE_JXL
GD::Image
gd_newFromJxl(packname="GD::Image", filehandle)
	char *	packname
	InputStream	filehandle
  PROTOTYPE: $$
  CODE:
	PERL_UNUSED_ARG(packname);
	RETVAL = GDIMAGECREATEFROMJXL(filehandle);
	if (!RETVAL)
            croak("gdImageCreateFromJxl error");
  OUTPUT:
	RETVAL

GD::Image
gdnewFromJxlData(packname="GD::Image", imageData)
	char *	packname
	SV *    imageData
  PROTOTYPE: $$
  PREINIT:
  ...
  CODE:
	...
	data = SvPV(imageData,len);
        ctx = newDynamicCtx(data,len);
	RETVAL = (GD__Image) gdImageCreateFromJxlCtx(ctx);
        (ctx->gd_free)(ctx);
        if (!RETVAL)
           croak("gdImageCreateFromJxlCtx error");
  OUTPUT:
	RETVAL
...
gdjxl(image, ...)
  ...
  PREINIT:
	...
	void*         data;
	int           size;
        int           lossless; // 0 for default (lossy)
        double        distance; // 1.0 is default, 0 = lossless equivalent
        int           effort;   // 7 is default, 1 (fastest) .. 9 (slowest)
  CODE:
    if (items > 1) {
      lossless = (int)SvIV(ST(1));
      distance = items > 2 ? (double)SvNV(ST(2)) : 1.0;
      effort   = items > 3 ? (int)SvIV(ST(3)) : 7;
      data = (void *) gdImageJxlPtrEx(image,&size,lossless,(float)distance,effort);
    }
    else
      data = (void *) gdImageJxlPtr(image,&size);
    if (data == NULL) {
      errormsg = perl_get_sv("@",0);
      if (errormsg != NULL)
        sv_setpv(errormsg,"libgd was not built with jxl support\n");
      else
        croak("gdImageJxlPtr error");
      XSRETURN_EMPTY;
    }
    RETVAL = newSVpvn((char*) data,size);
    gdFree(data);
...
int
gdtrueColorToPaletteSetMethod(image, method, speed=0)
	GD::Image	image
	int		method
	int		speed
  PROTOTYPE: $$;$
  CODE:
    RETVAL = gdImageTrueColorToPaletteSetMethod(image,method,speed);
  OUTPUT:
    RETVAL

void
gdtrueColorToPaletteSetQuality(image, min_quality, max_quality)
	GD::Image	image
	int		min_quality
	int		max_quality
  PROTOTYPE: $$$
  CODE:
    gdImageTrueColorToPaletteSetQuality(image,min_quality,max_quality);
...
# UltraHDR support (libgd >= 2.4.0). gdUhdrImage is a distinct opaque
# handle, not a gdImagePtr: it carries the SDR base image plus its HDR
# gain map, which a plain GD::Image cannot represent. Use getSdr() to
# obtain a viewable/saveable SDR GD::Image once gain-map-preserving
# transforms are no longer needed.

MODULE = GD		PACKAGE = GD::UHDR	PREFIX=gdUhdr

#ifdef HAVE_UHDR

bool
gdUhdrisAvailable(packname="GD::UHDR")
	char *	packname
  PROTOTYPE: $
  CODE:
    PERL_UNUSED_ARG(packname);
    RETVAL = (bool)gdUhdrIsAvailable();
  OUTPUT:
    RETVAL

GD::UHDR
gdUhdrnewFromFile(packname="GD::UHDR", filename, format=GD_UHDR_FORMAT_JPEG)
	char *	packname
	char *	filename
	int	format
  PROTOTYPE: $$;$
  ...
  CODE:
    ...
    RETVAL = gdUhdrImageCreateFromFile(filename,format,&err);
    if (!RETVAL)
      croak_uhdr("gdUhdrImageCreateFromFile",&err);
  OUTPUT:
    RETVAL

GD::UHDR
gdUhdrnewFromData(packname="GD::UHDR", imageData, format=GD_UHDR_FORMAT_JPEG)
	char *	packname
	SV *	imageData
	int	format
  PROTOTYPE: $$;$
  ...
  CODE:
    ...
    data = SvPV(imageData,len);
    memset(&err,0,sizeof(err));
    RETVAL = gdUhdrImageCreateFromPtr((int)len,(void*)data,format,&err);
    if (!RETVAL)
      croak_uhdr("gdUhdrImageCreateFromPtr",&err);
  OUTPUT:
    RETVAL
...
gdUhdrhasGainMap(im)
  ...
  PROTOTYPE: $
  CODE:
    RETVAL = (bool)gdUhdrImageHasGainMap(im);
  OUTPUT:
    RETVAL

void
gdUhdrresize(im, width, height)
	GD::UHDR	im
	int	width
	int	height
  PROTOTYPE: $$$
  ...
  CODE:
    memset(&err,0,sizeof(err));
    if (gdUhdrImageResize(im,width,height,&err) != GD_UHDR_SUCCESS)
      croak_uhdr("gdUhdrImageResize",&err);

void
gdUhdrcrop(im, left, top, width, height)
	GD::UHDR	im
	int	left
	int	top
	int	width
  ...
  CODE:
    memset(&err,0,sizeof(err));
    if (gdUhdrImageCrop(im,left,top,width,height,&err) != GD_UHDR_SUCCESS)
      croak_uhdr("gdUhdrImageCrop",&err);

void
gdUhdrrotate(im, degrees)
	GD::UHDR	im
	int	degrees
  PROTOTYPE: $$
  PREINIT:
  ...
  CODE:
    memset(&err,0,sizeof(err));
    if (gdUhdrImageRotate(im,degrees,&err) != GD_UHDR_SUCCESS)
      croak_uhdr("gdUhdrImageRotate",&err);

void
gdUhdrmirror(im, axis)
	GD::UHDR	im
	int	axis
  PROTOTYPE: $$
  PREINIT:
  ...
  CODE:
    memset(&err,0,sizeof(err));
    if (gdUhdrImageMirror(im,axis,&err) != GD_UHDR_SUCCESS)
      croak_uhdr("gdUhdrImageMirror",&err);

void
gdUhdrfile(im, filename, format=GD_UHDR_FORMAT_JPEG, quality=90)
	GD::UHDR	im
	char *	filename
	int	format
	int	quality
  ...
  CODE:
    memset(&err,0,sizeof(err));
    if (gdUhdrImageFile(im,filename,format,quality,&err) != GD_UHDR_SUCCESS)
      croak_uhdr("gdUhdrImageFile",&err);

SV*
gdUhdrwrite(im, format=GD_UHDR_FORMAT_JPEG, quality=90)
	GD::UHDR	im
	int	format
	int	quality
  PROTOTYPE: $;$$
  ...
  CODE:
    memset(&err,0,sizeof(err));
    data = gdUhdrImageWritePtr(im,&size,format,quality,&err);
    if (data == NULL)
      croak_uhdr("gdUhdrImageWritePtr",&err);
    RETVAL = newSVpvn((char*) data,size);
    gdFree(data);
  OUTPUT:
    RETVAL
...
gdUhdrgetSdr(im)
  ...
  CODE:
    memset(&err,0,sizeof(err));
    RETVAL = gdUhdrImageGetSdr(im,&err);
    if (!RETVAL)
      croak_uhdr("gdUhdrImageGetSdr",&err);
  OUTPUT:
    RETVAL
